import { Deceased, Heir, Calculator } from '../src/core'
...
describe('normal case',()=>{
  ...
  it('should 234: ashabah got 0',()=>{
    const calc = new Calculator(new Deceased(_234.deceased))
    _234.heirs.forEach(heir=>{
      calc.push(new Heir(heir))
    })
    ...
    expect(calc.calculation).toStrictEqual([new Fraction(1/2), new Fraction(1/6), new Fraction(1/3), new Fraction(0)])
  })


  it('should only: only wife',()=>{
    const calc = new Calculator()
    calc.push(new Heir({gender: false, isPartner: true, relation: Relation.other}))
    calc.push(new Heir({gender: false, isPartner: false, relation: Relation.siblingMother}))
    calc.push(new Heir({gender: false, isPartner: false, relation: Relation.other}))
...
describe('umaratain case',()=>{
  it('case 1: husband',()=>{
    const calc = new Calculator(new Deceased({gender: false}));
    calc.push(new Heir({gender: true, relation: Relation.none, isPartner: true}));
    calc.push(new Heir({gender: true, relation: Relation.parent, isPartner: false}));
    calc.push(new Heir({gender: false, relation: Relation.parent, isPartner: false}));
...
describe('siblings case',()=>{
  it('should akdariyah',()=>{
    ...
    const calc = new Calculator(new Deceased(data.deceased))
    data.heirs.forEach(heir=>{
      calc.push(new Heir(heir))
    })

   const akdariyah = (!calc.deceased.gender && calc.partner > 0 && calc.male(1,1) === 1 && calc.female(1,0) === 1 && calc.female(2,0) === 1 )
  ...
  it('keadaan kedua: 180 muqasamah',()=>{
    ...
    const calc = new Calculator(new Deceased(_180.deceased));
    _180.heirs.forEach(heir=>{
      calc.push(new Heir(heir))
    })
  ...
  it('should saudara pr kandung dan saudara seayah 189: 1/3',()=>{  //sisters share not subtructed
    ...
    const calc = new Calculator(new Deceased(_189.deceased));
    _189.heirs.forEach(heir=>{
      calc.push(new Heir(heir))
    })
  ...
  it('should 191: 1/3 rest',()=>{ //sisters share not subtructed
    const data: Data= {
      deceased: {gender: false},
      heirs: [
        {gender: false, relation: Relation.parent},
...
describe('radd case',()=>{
  it('should 4th case 225: there\'s partner, differ share',()=>{
    ...
    const calc = new Calculator(new Deceased(_225.deceased))
    _225.heirs.forEach(heir =>{
      calc.push(new Heir(heir))
    })
    expect(calc.calculation).toEqual([
      new Fraction(1/4),
