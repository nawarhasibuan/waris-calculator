...
class Calculator{
  ...
  public set deceased(deceased : Deceased) {
    if (this.partner > 0) {
      const heir: Heir = this.heirs.find(heir => heir.isPartner) || new Heir({gender: !deceased.gender, isPartner: true, relation: Relation.none})
  ...
  public get radd() : boolean {
    return this.remaining > 0 && this.noAshabah && this.remaining < this.baseProblem
  }
  ...
  public get siblingCase() : boolean {
    return this.noBoy && this.male(1,0) === 0 && !this.noAshlun && (this.male(2,0) + this.male(2,1) + this.female(2,0) + this.female(2,1)) > 0
  }  
  ...
  public get umaratain() : boolean {
    ...
    return this.noFarun && siblings < 2 && this.partner > 0 && (this.male(1,0)*this.female(1,0) + this.male(1,1)*this.female(1,1) + this.male(1,2)*this.female(1,2)) > 0
  }
  ...
  public get noAshabah() : boolean {
    ...
    return !found && (this.kalalah || (this.female(2,0) + this.female(2,1)) === 0)
  }
    ...
    if (this.umaratain) {
      ...
      this.heirs.forEach((heir, idx) => {
        if (heir.isPartner || heir.power !== 1 ) {
          ...
          saham[idx] = this.mahjub(idx) ? this.share(idx) : (heir.gender ? remain.mul(2/3) : remain.mul(1/3))
        }
      })
       ...
       * akdariyah
       */
      const akdariyah: boolean = !this.deceased.gender && this.partner > 0 && this.male(1,1) === 1 && this.female(1,0) === 1 && this.female(2,0) === 1 && this.heirs.length === 4
      if (akdariyah) {
        this.heirs.forEach((heir, idx)=>{
          if (heir.isPartner){
            saham[idx] = new Fraction(1/3)
          } else if (heir.power === 1) {
            saham[idx] = heir.gender ? new Fraction(1/8) : new Fraction(1/3)
          } else {
      ...
      const muqasamah = new Fraction(2/(2*this.male(1,1) + 2*this.male(2,0) + 2*this.male(2,1) + this.female(2,0) + this.female(2,1)))
      let remainingNoSibling: Fraction = new Fraction(0);
      this.heirs.forEach((heir,idx) => {
        //sister and grand father exclude
        remainingNoSibling = ((heir.code === 20 || heir.code === 21) && !heir.gender) || (heir.code === 11 && heir.gender) ? remainingNoSibling : remainingNoSibling.add(this.share(idx))
      })
      remainingNoSibling = new Fraction(1).sub(remainingNoSibling);
      grandPa = muqasamah.mul(remainingNoSibling).compare(remainingNoSibling.div(3)) > 0 ? (muqasamah.mul(remainingNoSibling).compare(1/6) > 0 ? 1 : 0) : (muqasamah.mul(remainingNoSibling).compare(1/6) > 0 ? -1 : (oneSixth.compare(remainingNoSibling.div(3))) > 0 ? 0 : -1)
      
      /**
       * grandpa share
       */
      const grandShare: Fraction = grandPa > 0 ? muqasamah.mul(remainingNoSibling) : (grandPa < 0 ? new Fraction(1/3).mul(remainingNoSibling) : new Fraction(1/6))
      ...
      if(aulSibling){
        ...
        this.heirs.forEach((heir, idx) => {
          sum = (heir.code === 11 && heir.gender) ? sum.add(grandShare) : sum.add(this.share(idx).div(this.friends(idx)))
          lcm = (heir.code === 11) ? lcm.lcm(grandShare.d) : lcm.lcm(this.share(idx).d);
        });
        const newProb = sum.mul(lcm.n)
        this.heirs.forEach((heir, idx) => {
          saham[idx] = this.share(idx).mul(lcm.n/this.friends(idx)).div(newProb)
        });
      } else {
        let sister: Fraction = new Fraction(0)
        this.heirs.forEach((heir, idx) => {
          sister = (heir.code === 20 || heir.code === 21) && !heir.gender ? sister.add(this.share(idx)) : sister
        })
        ...
        this.heirs.forEach((heir, idx) => {
          if (heir.power === 1 && heir.darajah > 0 && heir.gender) {
            ...
            if(this.ashabah(idx)){
              ...
              saham[idx] = heir.gender ?  rest.mul(2/siblings) : rest.div(siblings)
            } else {
              saham[idx] = this.share(idx).div(this.friends(idx))
            }
          }
      ...
      this.heirs.forEach((heir, idx) => {
        ...
        if (this.mahjub(idx)) {
          ...
          if (heir.isPartner) {
            saham[idx] = this.share(idx).div(this.friends(idx)).add(this.remaining/(this.baseProblem*this.dzawilArham))
          } else if (heir.power < 3) {  //child, parent, sibling
            saham[idx] = this.share(idx).add(this.share(idx).mul(n).div(sum.mul(n)).mul(this.remaining/this.baseProblem)).div(this.friends(idx))
          } else {  // rahmi: share + sisa/(base * friends)
            saham[idx] = this.share(idx).add(this.remaining/(this.friends(idx)*this.baseProblem))
          }
        }
      })
    } else if(this.aul) {
      ...
      this.heirs.forEach((heir, idx) => {
        saham[idx] = this.share(idx).mul(this.baseProblem/this.friends(idx)).div(newProb)
      });
    } else { //normal case
      this.heirs.forEach((heir, idx) => {
        if(this.ashabah(idx)){  //ashabah
          let a = heir.gender ? 2 : 1
          let b = heir.gender ? 1 : 2
          let siblings =  a*this.friends(idx) + b*this.siblings(idx)
          saham[idx] = heir.gender ?  new Fraction(this.remaining/this.baseProblem).mul(2/siblings) : new Fraction(this.remaining/this.baseProblem).div(siblings)
        } else {  //others
          saham[idx] = this.share(idx).div(this.friends(idx))
        }
      });
  ...
  public push(heir: Heir) {
    if (heir.isPartner){
      ...
      if ( this.deceased.gender && this.partner > 4 || (!this.deceased.gender && this.partner > 0) ) {
        throw new Error(`Partner not allowed greater then ${this.partner}`)
      }
    }
    if (heir.power > 3 && heir.darajah > 3) {
      throw new Error('Not a heir')
    }
    if (heir.power === 1 && heir.darajah === 0 && (heir.gender ? this.male(1,0) : this.female(1,0)) > 0) {
      throw new Error(`deceased only has 1 (one) ${heir.gender ? 'father' : 'mother'}.`)
    }
    if (heir.power === 2){
      if (heir.darajah === 2) {
  ...
  public heir(idx: number): Heir {
    return new Heir({gender: this.heirs[idx].gender, relation: this.heirs[idx].relation, isPartner: this.heirs[idx].isPartner, name: this.heirs[idx].name})
  }
  ...
  public share(idx: number): Fraction {
    ...
    } else if (!this.mahjub(idx) && !this.ashabah(idx)) { //not a mahjub
      if (heir.gender) {
        //male
        if (heir.power === 1) { // father-grand father
          x = x.add(1/6)
        } else {
        ...
        switch (heir.power*10 + heir.darajah) {
          ...
          case 21: //sister in father
            if (!this.female(2,0)) { //replace sister place
              x = this.lonly(idx) ? x.add(1/2) : x.add(2/3)
            } else if (this.female(2,0) === 1) {  //perfection 2/3 for sisters                      
              x = x.add(1/6)
            }
            break;
  ...
  public ashabah(idx: number): boolean {
    ...
    const bilGhairi = heir.power < 4 && (this.male(heir.power, heir.darajah) > 0 || (heir.power === 0 && heir.darajah > 0 && !this.noBoy));
    const maalGhairi = this.noBoy && !this.noFarun && heir.power === 2 && heir.darajah < 2;
    if (heir.power === 1) { //father-grand father
  ...
  public mahjub(idx: number): boolean {
    let found = true
    const heir = this.heirs[idx]
    if (heir.power > 3) { //liberator - dzawil arham - none family partner
    ...
    if (heir.gender) { //male
      ...
      if (heir.power > 2) {
        ...
        found = found || (!this.kalalah && (this.female(2,0) > 0 || this.female(2,1) > 0)) //sister's ashabah maal ghairi
      }
      ...
      let n = heir.power === 2 && heir.darajah > 2 ? heir.darajah - 1 : heir.darajah
      while (i < n && !found) {
      ...
      if (heir.power === 2) {
        switch (heir.darajah) {
          ...
          case 1: //brother in father                        
            found = this.male(1,0) > 0 || !this.noBoy || this.male(2,0) > 0 || (!this.noFarun && this.female(2,0) > 0)
            break;
      ...
      switch (connection) {
        ...
        case 21:    //sister father
          ...
          found = !this.noBoy || (this.male(1,0) > 0) || (this.male(2,0) > 0) || (this.female(2,0) === 1 && !this.noFarun) || (this.female(2,0) > 1 && this.siblings(idx) === 0)
          break;
  ...
  public lonly(idx: number): boolean {
    const heir = this.heirs[idx]
    return heir.gender? this.male(heir.power, heir.darajah) === 1 : this.female(heir.power, heir.darajah) === 1
  }
    ...
    if (heir.isPartner) {
      ...
      if (heir.power === 2){
        if (heir.darajah === 2) {
          ...
          let x = heir.gender ? heir.darajah-1 : heir.darajah
          return heir.gender ? this.male(heir.power, x) : this.female(heir.power, x)
        }
      }
      return heir.gender ? this.male(heir.power, heir.darajah) : this.female(heir.power, heir.darajah)
    }
  }
  ...
  public siblings(idx: number): number {
    ...
    if (heir.isPartner && heir.power > 3) {
      ...
      return heir.gender ?  this.female(heir.power, heir.darajah) : this.male(heir.power, heir.darajah)
    } else {
      return 0
    }
  }
